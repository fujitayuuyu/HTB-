...
* よく使用されているシェル
  * Zsh - macOSやkali linux、Parrot等で利用されている
  * Bash - linux系全般
  * sh - 始めにスペースを入れることで、ヒストリが残らないとかあるシェル(普通のシェルでもヒストリは、消せるからな)
  * rBash - 制限付きのBashシェル。これを用いることで特定のコマンドしか使えないユーザを作成したりできる。
...
### ② シェルの設定ファイル
シェルの設定は、各ユーザのホームディレクトリまたは、/etc/配下にある。シェル実行時の環境変数を設定しているところだよ。

#### わかればいいこと
* 個人でユーザのシェル設定を変更したいときは、`~/.bashrc`(zshなら`~/.zshrc`)を編集する。
* 全ユーザのシェルに設定を変更したいときは、`/etc/bashrc`(zshなら`/etc/zshrc`)を編集する。
* シェルの設定は、環境変数の設定が主に行われる。
* 設定を変更した後そのファイルを適用するために`sorce`コマンドを利用して適用する。
  * sorceコマンド - ファイル内のコマンドを現在のシェルで行うコマンド。だから、決して読み込むためのコマンドではない。
  > ```
  > sorce ~/.bashrc
  > ```
#### それぞれのシェルの設定ファイル利用の例
* ~/.zshrc - 自分のログインユーザ用のオリジナルプロンプトを適用するとき
* /etc/zshrc - プロキシ設定をしたいとき ：参考ＵＲＬ https://zenn.dev/junkor/articles/ca961ca70f441a
> ```
> $ vi /etc/zshrc
>
> ********** 省略 *************
...
* 参考：https://www.wantanblog.com/entry/2020/11/03/174501
...
## --コラム-- ~/.bashrc、/etc/bashrc、永続化に使えるんじゃね(攻撃者視点)
### ① 永続化に利用できそうな点
...
### ② 問題点
問題点としては、~/.bashrcも/etc/bashrcもユーザがログインしてくれないと実行されない。それならsystemdでサービスとしてやったりcron使った方が安定するし、セッションが切れたときでも、再度繰り返すようにすればいいので楽。
### ③ 結論
* **永続化には向かないんじゃね**
* ほんとうに、systemdやcronがめちゃくちゃに監視されているとかほかの永続化に利用できる手段がないときにしか使わなそう
## ---コラム---　コマンドログが残るせるか？ (防御者視点)
...
あと、環境変数を制限する方法が、制限付きシェル以外見つからなかったのであきらめてくれ
...
「-m tcp」を使用することで、TCP特有のマッチング機能やオプション(フラグとか)が利用でき、ルールの柔軟性や詳細な制御が可能になる。一方で、「-m」を使わない場合は、基本的なルールのみに限られる。
...
ip routeを用いることでルーティングが実装され、内部ネットワーク外部ネットワークの切り分けや、  
ネットワークルーティングが行えるようになりゅ。

* だから、ラズパイにraspi osを入れて、追加のLANポートをつければ、FW機器が作れる！！！  
また、dockerとか仮想マシンとかの仮想上でFWを立てることができる！！

#### ② 結局のところ
VyOSとかの既存のルータ用OSとか、既存のdockerイメージを入れてやればらくなんだけどね（笑）
...
* sudoによるコマンド実行のログもデフォルトで残るよ。
* ssh,ftp,smtpなどのネットワークサービスの認証も残るよ
#### アプリケーションログ
