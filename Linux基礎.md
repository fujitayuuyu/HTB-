...
* 方法： 各ユーザの~/.bashrcで$PROMPT_COMMAND環境変数にコマンドログをsyslogdに転送するスクリプトを埋め込む
> ```
> $ vi ~/.bashrc  (デフォルトでPROMPT_COMMANDの行がないんだったら追記「>>」でいいかもね)
> 
> **** 省略 *****
...
* 問題点：~/.bashrcを変更または、削除されてしまうと、ログを取得できない。sh等の別のシェルに行かれるとログを取得できない。環境変数$HISTSIZEを0にされてもログを取得できない。
#### ~/.bashrcについて
こいつを削除できないようにすればいいやん(読込権限のみ)と思ったが、こいつは、ログイン時に、コマンドのエイリアスや、環境変数、関数の設定を行うので、実行権限が必要だし、書き込み権限がないと、環境変数とかエイリアスとかを変更したとき反映されなくなるからダメなんだよね。
